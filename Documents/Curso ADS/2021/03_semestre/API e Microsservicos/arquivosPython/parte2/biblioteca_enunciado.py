...
def todos_alunos():
    with engine.connect() as con:   #conectar ao banco de dados
        ...
        resultados = []
        while True: # while que roda pra sempre
            result = rs.fetchone()
            # verifico se devo parar
            if result == None: # devo parar
                break
...
#         nome tabela
#                     valor que eu quero colocar na coluna 
#                                     linhas que quero editar
def empresta_livro(id_aluno, id_livro):
    with engine.connect() as con:   #conectar ao banco de dados
        sql_create = text ("UPDATE livro SET id_aluno = :id_do_aluno WHERE id_livro = :id_do_livro")
        con.execute(sql_create, id_do_aluno=id_aluno, id_do_livro=id_livro)
...
# NULL = NULL não existe sql, o correto é usar ISNULL quando for vazio o id solicitado
def todos_parados():
    with engine.connect() as con:   #conectar ao banco de dados
...
# SELECT Aluno.id, Aluno.nome, Livro.descricao 
# 	FROM Aluno INNER JOIN Livro 
#     ON Aluno.id = Livro.id_aluno
#     WHERE Aluno.nome = "Lucas Mendes"

def livros_do_aluno(nome):
    with engine.connect() as con:   #conectar ao banco de dados
        sql_consulta = text ('''SELECT Aluno.id, Aluno.nome, Livro.descricao
                             FROM Aluno INNER JOIN Livro ON Aluno.id = Livro.id_aluno
